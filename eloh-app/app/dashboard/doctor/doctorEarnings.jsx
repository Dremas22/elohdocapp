...
const Earnings = ({ data }) => {
  ...
  const earningsUpdatedAt = data?.earningsUpdatedAt;

  const formattedDate = convertTimestamp(earningsUpdatedAt);
  return (
    <>
      <div className="text-[#66e4ff] flex flex-col md:flex-row items-center justify-center gap-8 p-8 w-auto md:w-full">
        ...
        {/* Last updated at (main container) */}
      </div>
      <p className="text-sm text-[#88c2ff] italic mb-4 text-center">
        Last earnings update: {formattedDate}
      </p>
    </>
  );
};
