    ...
    <section className="mt-6 bg-white text-gray-800 border border-gray-200 rounded-3xl p-6 shadow-md">
      ...
      <div className="space-y-5">
        {groupedMedicalSections.map((group, index) => {
          const fieldError = errors[group.field];
          const otherFieldKey = `other${group.field.charAt(0).toUpperCase() + group.field.slice(1)}`;
          const otherFieldError = errors[otherFieldKey];
          ...
          return (
            ...
            >
              ...
              {/* Always visible error message (below header) */}
              {fieldError && (
                <p className="text-sm text-red-600 px-6 pb-2">
                  {fieldError}
                </p>
              )}
                  ...
                  {medicalHistory[group.field]?.includes("Other") && (
                    <div className="bg-[#f9fafb] rounded-xl p-3 border border-gray-100 sm:col-span-2">
                      ...
                      <input
                        ...
                        value={medicalHistory[otherFieldKey] || ""}
                        onChange={(e) => handleOtherChange(group.field, e.target.value)}
                        className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-2 focus:ring-[#0077b6] outline-none"
                      />
                      {/* Show error for "Other" input */}
                      {otherFieldError && (
                        <p className="text-sm text-red-600 mt-1">{otherFieldError}</p>
                      )}
                    </div>
                  )}
                </div>
