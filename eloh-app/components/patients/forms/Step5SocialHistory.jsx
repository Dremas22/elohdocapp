...
const Step5SocialHistory = ({ formData, setFormData, errors }) => {
  ...
  const hasSectionError = (fields) =>
    fields.some(
      (f) =>
        errors[f] ||
        errors[`other${f.charAt(0).toUpperCase() + f.slice(1)}`]
    );
    ...
    <section className="mt-6 bg-white text-gray-800 border border-gray-200 rounded-3xl p-4 sm:p-6 shadow-md">
      ...
      <div className="space-y-5">
        {groupedSections.map((group, index) => {
          const sectionHasError = hasSectionError(group.fields);

          return (
            <div
              key={group.title}
              className={`border border-gray-200 rounded-2xl shadow-sm bg-gray-50 hover:shadow-md transition-all duration-300 ${sectionHasError ? "border-red-300" : ""
                }`}
            >
              {/* Collapsible Header */}
              <button
              ...
              {expanded === index && (
                <div className="px-4 sm:px-6 pb-4 sm:pb-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-200 bg-white rounded-b-2xl">
                  {group.fields.map((field) => (
                    <div
                      key={field}
                      className={`bg-[#f9fafb] rounded-xl lg:-ml-5 p-4 sm:p-4 border border-gray-100 w-[26vh] ${["livingSituation", "drugs", "hobbies"].includes(field)
                        ? "lg:w-[45vh]"
                        : ""
                        }`}
                    >
                      ...
                      <div className="space-y-2">
                        {fieldOptions[field].map((option) => (
                          <label
                            key={option}
                            className={`flex flex-wrap items-center justify-between bg-white hover:bg-[#f1f5f9] rounded-lg p-2 sm:p-3 border border-gray-200 transition-all duration-200 font-semibold text-gray-900 w-full ${["livingSituation", "drugs", "hobbies"].includes(
                              field
                            )
                              ? "lg:w-[35vh]"
                              : ""
                        ...
                        {Array.isArray(socialHistory[field]) &&
                          socialHistory[field].includes("Other") && (
                            <input
                              ...
                              value={
                                socialHistory[
                                `other${field.charAt(0).toUpperCase() +
                                field.slice(1)
                                }`
                                ] || ""
                              }
                        ...
                        {/* Inline validation errors (only visible when expanded) */}
                        {errors[field] && (
              ...
              {/* Always-visible error summary below dropdown */}
              {!expanded && sectionHasError && (
                <p className="text-sm text-red-600 px-5 pb-3">
                  Please review missing details in this section.
                </p>
              )}
            </div>
          );
