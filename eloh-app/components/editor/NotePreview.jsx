    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 px-4">
      ...
      >
        ...
        <div className="flex justify-between items-center border-b pb-4 mb-5 mt-3 border-gray-300">
          <div className="w-20 h-20 rounded-full bg-[#caf0f8] text-[#03045e] font-semibold flex items-center justify-center text-sm shadow-inner">LOGO</div>
        ...
        <div className="text-gray-800 font-serif text-[16px] sm:text-[17px] leading-relaxed text-justify">
          ...
          {noteType === "sickNotes" && (
            <>
              <h2 className="text-2xl font-bold text-center text-[#061339] underline mb-4">Sick Note</h2>
              <p className="text-right mb-4">Date: <span>{formatDate(previewData.content?.startDate)}</span></p>
              <p className="mb-2">To Whom It May Concern,</p>
              <p className="mb-2">This letter serves as an official medical excuse for:</p>
              <p className="text-center font-mono border-b border-gray-400 py-1 mb-2">
                {previewData?.patientName || "_____________________________"}
              </p>
              <p className="mb-2">
              ...
              <p className="mb-6">If further accommodations or verification are required, please contact my office.</p>
              <p className="font-semibold">Sincerely,</p>
              <p className="border-b border-gray-500 inline-block min-w-[250px] py-1">{docName}</p>
          ...
          {noteType === "prescriptions" && (
            <div className="relative">
              <h2 className="text-2xl font-bold text-center text-[#03045e] mb-6 underline">Prescription Note</h2>
              <div className="absolute inset-0 flex justify-center items-center opacity-10 text-[10rem] text-[#03045e] pointer-events-none">℞</div>
              <div className="relative bg-gray-50 p-6 rounded-xl border border-gray-300 space-y-5 z-10">
                ...
                <div>
                  ...
                  {Array.isArray(previewData.content?.medications) ? (
                    <ul className="list-disc pl-5">
                      {previewData.content.medications.map((item, i) => <li key={i}>{item}</li>)}
                    </ul>
                  ) : (
                ...
                <p>
                  ...
                  {previewData.content?.instructions || <span className="italic text-gray-500">No instructions provided.</span>}
                </p>
                <div className="flex justify-between mt-10">
                  <div className="text-sm text-gray-700">
                    <p className="font-semibold text-[#03045e]">Dr. {docName}</p>
          ...
          {noteType === "generalNotes" && (
            <>
              <h2 className="text-2xl font-bold text-center text-[#03045e] underline mb-6">Patient Medical Report</h2>
              <div className="grid sm:grid-cols-2 gap-4 border border-gray-300 p-4 rounded-xl mb-6">
                <div>
                  <p className="text-sm text-gray-600">Patient Name</p>
                  <p className="text-base font-medium">{previewData.content?.patientName || "________________________"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Date Created</p>
                  <p className="text-base font-medium">{formatDate(previewData.createdAt)}</p>
                </div>
                <div>
              ...
              <div className="bg-gray-50 border border-gray-200 p-6 rounded-xl whitespace-pre-line text-base">
                <h3 className="text-lg font-semibold text-[#023e8a] mb-2">Doctor's Notes:</h3>
